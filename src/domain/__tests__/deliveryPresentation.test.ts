...
const sync = (status: 'QUEUED' | 'SENDING' | 'SYNCED' | 'CONFLICT' | 'FAILED' | 'NEEDS_USER_REVIEW', lastErrorCode?: any) => ({ syncType: 'publication' as const, envelope, status, type: 'workout' as const, lastErrorCode });
...
describe('truthful workout delivery presentation', () => {
  ...
  it('shows the primary on-phone state for an exact acknowledgement', () => {
    const result = presentWorkoutDelivery({ workout, syncRecord: sync('SYNCED'), acknowledgements: [ack()] });
    expect(result?.phase).toBe('AVAILABLE_IN_APPS'); expect(result?.title).toBe('On your phone');
    expect(result?.destinations.find(d => d.applicationId === 'HUMAN_HIIT')?.state).toBe('NOT_YET_RECEIVED');
  });
  it('maps attention, automatic retry and superseded history to calm primary states', () => {
    expect(presentWorkoutDelivery({ workout, syncRecord: sync('CONFLICT', 'OWNERSHIP_CONFLICT') })?.phase).toBe('CONFLICT');
    expect(presentWorkoutDelivery({ workout, syncRecord: sync('NEEDS_USER_REVIEW', 'PERMISSION_DENIED') })?.primaryStatus).toBe('NEEDS_ATTENTION');
    const retryable = presentWorkoutDelivery({ workout, syncRecord: sync('FAILED', 'NETWORK_RETRYABLE') });
    expect(retryable?.primaryStatus).toBe('SENDING'); expect(retryable?.canRetry).toBe(false);
    expect(presentWorkoutDelivery({ workout, syncRecord: sync('SYNCED'), latestRevision: 2 })?.phase).toBe('SUPERSEDED');
  });
  it('does not invite a retry when an immutable published version is unavailable', () => {
    const result = presentWorkoutDelivery({ workout, syncRecord: sync('FAILED', 'UPLOAD_FAILED'), publishedVersionAvailable: false });
    expect(result?.primaryStatus).toBe('NEEDS_ATTENTION'); expect(result?.canRetry).toBe(false);
    expect(result?.detail).toContain('published version is unavailable');
  });
});
