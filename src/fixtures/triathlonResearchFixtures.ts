...
export interface ResearchWeek { weekNumber: number; phaseName: string; recoveryWeek: boolean; swimMinutes: number; bikeMinutes: number; runMinutes: number; strengthMinutes: number; multisportMinutes: number; recoveryMinutes: number; requiredTotalMinutes: number; optionalMinutes: number; days: ResearchDay[] }
export interface ResearchPlan { planId: string; name: string; durationWeeks: number; prerequisites: unknown; phases: Array<{ phaseId: string; phaseName: string; phaseObjective: string }>; weeks: ResearchWeek[]; evidenceReferenceIds: string[]; provenance: { fixtureClassification: "RESEARCH_CANDIDATE"; sportsScienceReviewRequired: true; medicalAdvice: false } }
