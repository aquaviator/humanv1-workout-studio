...
export function TriathlonResearchLibrary({ onClone }: { onClone: (plan: ResearchPlan) => void | Promise<void> }) {
  ...
  return <section aria-labelledby="research-library-title" className="mt-10 border-t border-hv-border pt-6">
    ...
    <article className="mt-4 rounded-lg border border-hv-border bg-hv-surface-1 p-4">
      ...
      <button type="button" onClick={() => onClone(plan)} className="mt-4 rounded bg-hv-primary px-3 py-2 text-white focus-visible:ring-2 focus-visible:ring-hv-primary">Create user-owned research draft</button>
    </article>
  </section>;
}
